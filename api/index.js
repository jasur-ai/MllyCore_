...
// Telegram kodini yaratish (15 daqiqa muddat bilan)
...
async function handleTelegramWebhook(req, res, db) {
  ...
  try {
    if (lower === '/start' || lower.startsWith('/start ')) {
      ...
      var codeSnap = await db.collection('telegramCodes').where('code', '==', code).limit(1).get();
      if (codeSnap.empty) {
    ...
    if (/^[A-Z0-9]{6}$/.test(text)) {
      var cs = await db.collection('telegramCodes').where('code', '==', text).limit(1).get();
      if (cs.empty) {
...
module.exports = async (req, res) => {
  ...
  if (_pathname.includes('telegram-webhook') || _pathname.includes('telegram_webhook') || (req.query && req.query.action === 'telegram-webhook')) {
    try {
      initAdmin();
  ...
  try {
    ...
    const SENSITIVE = new Set([
      ...
      'send-chat', 'create-workspace', 'forgot-password',
    ]);
